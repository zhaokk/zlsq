...
export const Display = observer(() => {
  ...
  const { mode, state, locked, screenOff, childLock, 
  ...
  if(state===State.IDLE){
    ...
    if(leftSec < 60) {
      mStr = String(s).padStart(2,"0");
      showSeconds = true;
    } else {
      mStr = String(m).padStart(2,"0");
    }
    sSmall = ":"+String(s).padStart(2,"0");
  }
    ...
    <div className={clsx("display-container", { "screen-off": screenOff, "flashing": isFlashingReward })}>
      <div className="status-bar">
        <div className="status-icons">
          ...
          <div className={clsx("icon", {active: icSmile})}>
      ...
      <div className="main-time">
        ...
        <div className="digit-group">
          ...
          <span className="unit-label">{showSeconds ? "SECS" : "MINS"}</span>
        </div>
      </div>
      ...
      <div className="progress-section">
        <div className="progress-bar">
          {Array.from({length: 21}).map((_, i) => (
            <div key={i} className={clsx("progress-segment", {filled: i < progressCount})}></div>
          ))}
        </div>
      </div>
    </div>
